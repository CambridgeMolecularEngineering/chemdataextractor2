...
solvent = (
    (
        ...
        + Optional((R("^(\+|&|and)$") | cc) + (nmr_solvent | chemical_name))
        + Optional(SkipTo(R("^([;:,\.\)]|at)$")))
...
temp_word = (I("room") + R("^temp(erature)?$") | R("^r\.?t\.?$", re.I))(
    "value"
).add_action(join)
...
shift_value = (Optional(R("^[\-–−‒]$")) + R("^δ?[\+\-–−‒]?\d+(\.+\d+)?\.?$"))(
    "shift"
).add_action(merge)
shift_error = (
    Optional(R("^[\-–−‒]$")) + R("^δ?[\+\-–−‒]?\d+(\.+\d+)?,\d+(\.+\d+)?\.?$")
)("shift").add_action(merge)
shift = (
...
coupling = (
    (
        R("^\d?J([HCNPFD\d,]*|cis|trans)$")
        + ZeroOrMore(R("^J?([HCNPFD\d,]*|cis|trans)$"))
...
number = (R("^\d+(\.\d+)?[HCNPF]\.?$") | (R("^\d+(\.\d+)?$") + R("^[HCNPF]\.?$")))(
    "number"
).add_action(merge)
...
assignment_options = (
    OneOrMore(
        ...
        | W("′")
        | chemical_name
        | R("^(C?quat\.?|Ac|Ar|Ph|linker|bridge)$")
    )
    + Optional(W("×") + R("^\d+$"))
)("assignment").add_action(join)
assignment = Optional(R("^\d{1,2}$")("number") + Optional(W("×")).hide()) + (
...
prelude = (
    (nucleus + Optional(R("^[\-–−‒]$")).hide() + nmr_name | nmr_name_with_nucleus)
