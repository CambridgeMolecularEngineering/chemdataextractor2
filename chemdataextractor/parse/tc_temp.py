...
class CurieTemperatureParser(BaseAutoParser):
    ...
    def root(self):
        ...
        unit_element = Group(construct_unit_element(self.model.dimensions).with_condition(match_dimensions_of(self.model))('units'))('curietemp')
        specifier = self.model.specifier('specifier')
        value_phrase = value_element(unit_element)('curietemp')
        chem_name = (cem | chemical_label | lenient_chemical_label)
        entities = (value_phrase | specifier | chem_name)
        root_phrase = OneOrMore(entities | Any().hide())('curietempphrase')
    ...
    def interpret(self, result, start, end):
        try:
            ...
            raw_value = first(result.xpath('./curietemp/value/text()'))
            raw_units = first(result.xpath('./curietemp/units/text()'))
...
class CurieTemperatureTableParser(AutoTableParser):
    ...
    def root(self):
        ...
        value_phrase = value_element_plain()('curietemp')
        chem_name = (cem | chemical_label | lenient_chemical_label)
        entities = (value_phrase | specifier | chem_name)
        root_phrase = OneOrMore(entities | Any().hide())('curietempphrase')
        self._root_phrase = root_phrase
